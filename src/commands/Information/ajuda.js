...
const { musicEvent, error, warn, botEvent } = require('../../Logger');
...
async function Ajuda(interaction) {
  ...
  try {
    if (commandName === 'ajuda') {
      ...
      botEvent('COMANDO_USADO', 'Ajuda', interaction.user.tag, interaction.guild?.name, commandName);
      
    }
  } catch (err) {
    error('ERRO_AO_EXECUTAR_COMANDO', 'Ajuda', err);
    const errorEmbed = new EmbedBuilder()
      .setColor('#DDBD86')
      .setDescription('<:error:1364257210745487422> Ocorreu um erro ao executar o comando. Tente novamente mais tarde.');
